...
## 🎬 Demo 用 Prompt（現場展示腳本）

以下腳本依序操作，能在 3 分鐘內把 Radial AI「發散 → 引用 → 收斂」的核心價值完整秀出來。建議**新開一個空白畫布**進行。

### 第 1 步 — 種下主幹（主時間線）
直接在底部輸入框送出：

```
用一句話解釋「第一性原理」，再給我三個能套用它的真實情境。
```

> 畫布左側長出 Node 1，右側展開完整回答 —— 展示「對話即節點」。

### 第 2 步 — 放射狀引用（長出分支）
在右側回答中**反白其中一個情境**（例如「SpaceX 降低火箭成本」）→ 按 **⌘K** 引用 → 在輸入框送出：

```
把剛剛引用的這個情境，拆解成一張可執行的行動清單。
```

> 新節點以**分支箭頭**接在被引用節點旁 —— 展示「精準引用、只送直系脈絡」。

### 第 3 步 — 再開一條互不干擾的分支
點回 Node 1，反白**另一個情境**，⌘K 引用後送出：

```
這個情境最容易失敗的三個假設是什麼？請逐一反駁。
```

> 同一張畫布上同時存在兩條獨立思路 —— 展示「打破線性限制」。

### 第 4 步 — DAG 多父匯合（收斂成新觀點）
**Shift+Click** 同時選取第 2、3 步生成的兩個節點（邊框轉粉紅）→ 送出：

```
綜合上面這兩條分支的結論，幫我寫出一段給投資人的 30 秒電梯簡報。
```

> 新節點以箭頭**同時連回兩個父節點**，AI 整合雙方脈絡 —— 這是 Radial AI 最殺的一招。

### 加碼 — AI Persona
點畫布左上角 **🤖**，角色設「**毒舌但專業的創投合夥人**」，再重送第 4 步，對比語氣差異。

---

> 💡 Demo 小撇步：用 **🌐 所有節點** Context 模式送一句「**幫我把這整張畫布總結成一頁筆記**」，展示跨分支綜合歸納。
