...
### v3（2026-06）
- **三天免費試用**：新 Google 登入者可免費試用開發者 API Key 3 天，無需自備金鑰
- **可編輯畫布標題**：進入畫布後左上角（GEM 自訂左側）顯示標題，點擊即可改名
- **懸浮輸入視窗可拖移／縮放**：位置與大小會記住（localStorage）
- **連線可編輯**：拖曳連線端點改接到其他節點（重接血親記憶）；點選連線出現 ✕ 可斷線（切斷血親記憶）
- **空白節點**：雙擊畫布空白處新增空白節點；下一題會從「目前選取的節點」分支
- **貼上原文**：把其他 LLM 的回答原文直接貼成新節點（不呼叫 API）——「答」一字不漏，「問」顯示「無」
- **誕生動畫**：上方功能列「▶ 誕生動畫」依節點編號依序亮起，呈現節點誕生順序
- **AI 自動命名**：用免費 Gemini（gemini-3.1-flash-lite）依「問＋引用＋答」自動命名節點標題，另有「✨ AI命名」一鍵重新命名全部
